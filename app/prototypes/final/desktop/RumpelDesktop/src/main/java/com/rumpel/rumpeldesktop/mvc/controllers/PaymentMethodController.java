...
/**
 *  Controller for the payment method dialog and all the functions
 */
public final class PaymentMethodController implements Initializable {
    @FXML
    ...
    private void deletePaymentMethod() {
        if (pmComboBox.getSelectionModel().getSelectedIndex() == -1) errorAndWait(getString("no_item_selected"));
            ...
            if (buttonType.isPresent() && buttonType.get() == ButtonType.OK) {
                ...
                if (new DAOBudget().budgetWithPmExists(selectedPm)) {
                    errorAndWait("budget exists with pm");
                    return;
                }
                if (new DAOBill().billWithPmExists(selectedPm)) {
                    errorAndWait("bill exists with pm");
                    return;
                }
