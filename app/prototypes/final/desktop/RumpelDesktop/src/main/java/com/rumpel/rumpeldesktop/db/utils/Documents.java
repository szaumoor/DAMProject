...
public final class Documents {
    /**
     * Private constructor with safeguard to avoid illegal instantiation through Java reflection
     */
    private Documents() {
     ...
     * @return the filter
     */
    public static Bson monthAndYearToFilter(final Year year, final Month month, final String field) {
        var date = Dates.fromZonedDate(LocalDate.of(year.getValue(), month.getValue(), 1).atStartOfDay(ZoneId.systemDefault()));
        int fixedYear = month == Month.DECEMBER ? year.getValue() + 1 : year.getValue();
        return and(gte(field, date), lt(field, LocalDate.of(fixedYear, month.plus(1), 1)));
    }
     ...
     * @return the filter
     */
    public static Bson yearToFilter(final Year year, final String field) {
        var localDate = LocalDate.of(year.getValue(), 1, 1);
        var date = Dates.fromZonedDate(localDate.atStartOfDay(ZoneId.systemDefault()));
        return and(gte(field, date), lt(field, Dates.fromZonedDate(localDate.plusYears(1).atStartOfDay(ZoneId.systemDefault()))));
    }
     ...
     */
    public static Bson fromYearToFilter(final Year year, String field) {
        var localDate = LocalDate.of(year.getValue(), 1, 1);
        var date = Dates.fromZonedDate(localDate.atStartOfDay(ZoneId.systemDefault()));
     ...
     */
    public static Bson yearIntervalToFilter(final Year beginningYear, final Year endYear, String field) {
        var firstDayBeginningYear = LocalDate.of(beginningYear.getValue(), 1, 1);
        var firstDayEndYear = LocalDate.of(endYear.getValue() + 1, 1, 1);
        var dateBeginning = Dates.fromZonedDate(firstDayBeginningYear.atStartOfDay(ZoneId.systemDefault()));
        var dateEnd = Dates.fromZonedDate(firstDayEndYear.atStartOfDay(ZoneId.systemDefault()));
        return and(gte(field, dateBeginning), lt(field, dateEnd));
    }

    /**
     * Generates a Bson query to retrieve budgets that fall within a specific interval.
     *
     * @param year  The year of the interval.
     * @param month The month of the interval.
     * @return The Bson query representing budgets within the specified interval.
     */
    public static Bson budgetsInInterval(final Year year, final Month month) {
        int fixedYear = month == Month.DECEMBER ? year.getValue() + 1 : year.getValue();
        return and(lt(END_DATE_FIELD, LocalDate.of(fixedYear, month.plus(1), 1)));
    }
}
