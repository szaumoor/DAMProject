...
/**
 * Controller for the budget dialog and all the functions
 */
public final class BudgetDialogController implements Initializable {
    @FXML
    private Label lblPm;
    @FXML
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        Spinners.setForNumbers(spinnerSoft, 0.0, Double.MAX_VALUE);
        Spinners.setForNumbers(spinnerHard, 0.0, Double.MAX_VALUE);

        CheckBoxes.bindStateToNodes(checkSoftLimit, true, spinnerSoft, lblSoftLimit);
        CheckBoxes.bindStateToNodes(checkPm, true, pmComboBox, lblPm);
        ...
        currComboBox.setOnKeyPressed(event -> { // attempt at search by key
        ...
        budgetsComboBox.valueProperty().addListener((observable, oldValue, newValue) -> {
            ...
            if (value != null) {
                ...
                lblSoftLimit.setDisable(!(softLimit > 0));
    ...
    public void insertBudget() {
        ...
        if (opInterval.isEmpty()) {
            errorAndWait(getString("invalid_budget_dates"));
            return;
        }
        ...
        if (budget.isEmpty()) {
            errorAndWait(getString("invalid_budget"));
            return;
        }
